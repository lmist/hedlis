...
[[docs/assets/cloak-logo-readme.png]]
...
Once the npm release is live, install =cloak= globally and verify the CLI:
...
During =npm install=, =cloak= now tries to:

- warm the pinned OpenCLI extension cache
- install the Patchright Chromium browser

If the browser download step ever gets skipped or fails in your environment, retry it manually:

#+begin_src sh
npx patchright install chromium
#+end_src

Until the npm release is live, use the =from source= section below.
